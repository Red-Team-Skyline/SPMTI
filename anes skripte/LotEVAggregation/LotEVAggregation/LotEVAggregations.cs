		...
		public OnPrepareFetchOutputArgs OnPrepareFetch(OnPrepareFetchInputArgs args)
		{
			...
			try
			{
				foreach (var lotKey in _parkingSources.Keys)
				{
					...
					double evOccupied = 0;
					int n = Math.Min(evRows.Count, 10);

					var parkingList = parkingTable.Values.Take(n).ToList();
					...
					for (int i = 0; i < n; i++)
					{
						...
						if (parkingRow.Length > 4 && parkingRow[4] != null)
						{
							...
							evOccupied += parkingValue >= 0.5 ? 1.0 : 0.0;
						}
					}

					double occupancyPercent = Math.Round((evOccupied / 10.0) * 100.0, 0);
